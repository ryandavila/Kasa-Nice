...
def test_recent_samples_filters_by_since(store):
    now = int(time.time())
    store.record("10.0.0.4", 5.0, 0.1, ts=now - 10_000)
    store.record("10.0.0.4", 8.0, 0.2, ts=now - 100)
...
def test_daily_totals_takes_max_per_local_day(store):
    ...
    store.record("10.0.0.4", 5.0, 0.2, ts=_local_noon(today))
    store.record("10.0.0.4", 5.0, 0.5, ts=_local_noon(today) + 3600)
    store.record("10.0.0.4", 5.0, 1.0, ts=_local_noon(two_days_ago))
...
def test_daily_totals_ignores_null_today_kwh(store):
    today = datetime.date.today()
    store.record("10.0.0.4", 5.0, None, ts=_local_noon(today))
...
def test_migrate_device_id_repoints_samples(store):
    now = int(time.time())
    store.record("10.0.0.4", 5.0, 0.1, ts=now - 100)
...
def test_today_kwh_by_device_takes_todays_max(store):
    ...
    store.record("dev-a", 5.0, 0.2, ts=_local_noon(today))
    store.record("dev-a", 5.0, 0.7, ts=_local_noon(today) + 3600)
    store.record("dev-b", 5.0, 0.3, ts=_local_noon(today))
    store.record("dev-a", 5.0, 9.9, ts=_local_noon(yesterday))  # excluded
...
def test_month_kwh_by_device_sums_daily_maxima(store):
    ...
    store.record("dev-a", 5.0, 0.4, ts=_local_noon(first))
    store.record("dev-a", 5.0, 0.6, ts=_local_noon(first) + 3600)
    store.record("dev-a", 5.0, 0.5, ts=_local_noon(second))
    store.record("dev-a", 5.0, 9.9, ts=_local_noon(prev_month))  # other month
...
def test_home_kwh_between_sums_devices_and_days(store):
    ...
    store.record("dev-a", 5.0, 0.5, ts=_local_noon(day0))
    store.record("dev-a", 5.0, 0.7, ts=_local_noon(day1))
    store.record("dev-b", 5.0, 0.2, ts=_local_noon(day0))
    store.record("dev-a", 5.0, 9.9, ts=_local_noon(before))  # excluded
...
def test_idle_draw_medians_overnight_window_only(store):
    ...
    store.record("dev-a", 1.0, None, ts=_local_hour(day, 2))
    store.record("dev-a", 3.0, None, ts=_local_hour(day, 3))
    store.record("dev-a", 5.0, None, ts=_local_hour(day, 4))
    store.record("dev-a", 99.0, None, ts=_local_hour(day, 14))
...
def test_idle_draw_excludes_samples_outside_window(store):
    ...
    store.record("dev-a", 5.0, None, ts=_local_hour(old, 3))
    ...
    class FakeRegistry:
        ...
        async def read_energy_snapshot(self, device_id):
            if device_id == metered.host:
                return EnergySnapshot(power_w=9.0, today_kwh=0.4)
